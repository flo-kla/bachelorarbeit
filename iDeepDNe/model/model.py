        ...
        with tf.device('/cpu:0'):
            ...
            if not W_initializer is None:
                ...
                if params.projection:
                    ...
                    embeddings_prior_reshape = tf.reshape(self.embeddings_prior, [-1, W_prior_shape[1]])
        ...
        with tf.device('/cpu:0'):
            ...
            if not W_initializer is None:
                ...
                if params.projection:
                    ...
                    embeddings_prior_reshape = tf.reshape(self.embeddings_prior, [-1, W_prior_shape[1]]) #removed .value
                    embeddings_prior_projected = tf.matmul(embeddings_prior_reshape, W_prior_proj)
                    self.embeddings_prior = tf.reshape(embeddings_prior_projected, [self.b_s[0], self.b_s[1], params.hidden_size])

                    embeddings_prior_bw_reshape = tf.reshape(self.embeddings_prior_bw, [-1, W_prior_shape[1]])
